...
var testimg = ['img/i2_p11_act3.png','img/i3_p11_act3.png','img/i4_p11_act3.png']
...
numero_pagina(11);
function fIniciar(){
    resp_correctas = ["sapo","Pulgarcilla","pececillos","hada","mujer"];
    persCuento = ["bruja","sapo","Pulgarcilla","pececillos","hada","dragón","ratón","mujer"];
...
function finitimg(){
    ...
    for (i = 0; i < cajasImg.length; i++) {
		...
		$('#'+cajasImg[i].id).html('<img class="img-responsive" src="'+ testimg[i] +'" alt="' + testimg[i] + '" width="227px" height="180px">');
    } 
}
    ...
    function validaArray(sel,ca,isImg){
        ...
        if(isImg == 1){ //1 isImg 0 noImg
            if(ca=='img/i3_p11_act3.png'){ //correcto
                validador=2;
            }else{
                //validadorImg = -1;
                validador = -1;
...
function calificar()
{
    ...
    if(respuestas.sum_resp < 5){
        alert("Debe escoger 5 respuestas de la primera pregunta");
        if(respuestas.sum_resp_img == 0){
