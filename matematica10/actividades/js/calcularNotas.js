...
function calcSeparatedActScoreSelectionRf(querySelectorSelect,corAns,idImputShowAns,totalTestPoints,snCormenIncor,reqShowAns){
    ansAnsSel=[],ansAnsCorSel=[],ansAnsIncorSel=[];
    var maxCorAns=corAns.length;
...
// #region FuncionesEquivalentes
function calcEqFunctions(selectorFrac1,selectorFrac2,maxCorAns,totalTestPoints,showAnswerToSpan=boolean,idImputShowAns){
    frac1=[],frac2=[],ansAnsCor=[],ansAnsIncor=[];
    var conAnsScore = getValuesEqFunctions(selectorFrac1,selectorFrac2);
    var score = (totalTestPoints * conAnsScore)/maxCorAns;
    if(showAnswerToSpan){
        showAnswer(idImputShowAns,score);
    }else{
...
function getValuesEqFunctions(selectorFrac1,selectorFrac2){
    var firstfraction = document.querySelectorAll(selectorFrac1);
    var secondfraction = document.querySelectorAll(selectorFrac2);
    Array.from(firstfraction).forEach((elementff,indexff) =>{
        Array.from(elementff.querySelectorAll('span')).forEach((elementffs,indexffs)=>{
            if(indexffs==0){
                frac1.push([elementffs.textContent]);
            }else{
                frac1[indexff].push(elementffs.textContent);
            }
        });
    });
    Array.from(secondfraction).forEach((elementff,indexff) =>{
        Array.from(elementff.querySelectorAll('span')).forEach((elementffs,indexffs)=>{
            if(indexffs==0){
                frac2.push([elementffs.textContent]);
            }else{
                frac2[indexff].push(elementffs.textContent);
            }
        });
    });
    return checkAnsEqFunctions(firstfraction);
}
function checkAnsEqFunctions(node){
    frac1.forEach((element,index)=>{
        var mult1 = element[0]*frac2[index][1];
        var mult2 = element[1]*frac2[index][0];
        if(mult1 == mult2){
            ansAnsCor.push(node[index].parentElement.id); 
        }else{
            ansAnsIncor.push(node[index].parentElement.id);
        }
    });
    return ansAnsCor.length;
}
// #endregion
// #region Calculo Preguntas Verdadero y Falso
function checkAnsSelectionVF(corAns,ansAnsSel,nodeList,snCormenIncor){ //snCormenIncor 0 se resta correctas menos incorrectas, 1 solo se toma en cuenta las correctas
    ansAnsSel.forEach((elementAA,indexAA)=>{
        if(JSON.stringify(corAns).indexOf(JSON.stringify(elementAA)) >= 0){
            ansAnsCorSel.push(nodeList[indexAA].id);
        }else{
...
function calcSeparatedActScoreVF(querySelector,corAns,idImputShowAns,totalTestPoints,showAnswerSN){
    ansAns=[],ansAnsCor=[],ansAnsIncor=[];
    var maxCorAns=corAns.length;
    var conAnsScore = getNodesElementsVF(querySelector,corAns);
    var score = (totalTestPoints * conAnsScore)/maxCorAns;
    if(showAnswerSN){
        showAnswer(idImputShowAns,score);
    }else{
...
function getNodesElementsVF(querySelector,corAns){
    //var selectorQuery = '.'+classNameContenedorRespSelected;
    var lastChildNode = document.querySelectorAll(querySelector);
    lastChildNode.forEach((element,index)=>{
        var txtSpan = element.querySelectorAll('span');
        var radio = element.querySelectorAll('input[type=radio]');
        var radioValue ;
        radio.forEach(elementR => {
            if(elementR.checked){
                radioValue = elementR.value;
            }
        });
        buildPar(txtSpan[0].textContent,radioValue);
        /*
        var attributeTypeRadio = element.getAttribute('type');
        if(element.nodeName == 'INPUT'){
            if(attributeType=='text'){
                buildPar(index,element.value);
            }else if(attributeType == 'radio'){
    ...
    return checkAnsVF(corAns,ansAns,lastChildNode);
}
function buildParVF(indexAns,valueAns){
    ansAns.push([indexAns,valueAns]);
}
function checkAnsVF(corAns,ansAns,nodeList){
    ansAns.forEach((elementAA,indexAA)=>{
        if(JSON.stringify(corAns).indexOf(JSON.stringify(elementAA)) >= 0){
            ansAnsCor.push(nodeList[indexAA].id);
        }else{
