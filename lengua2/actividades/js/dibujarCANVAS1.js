	...
	window.onload=function() {
		...
		coloresdefondo=contenedoropciones1.getElementsByClassName("celdascoloresdefondo");
		for (var x=0; x<coloresdefondo.length; x++) {
		...
		prueba1.width=window.innerWidth*80/100;
		altoinicial=prueba1.height=prueba1.width/2;		
		//iniciamos el entorno de dibujo 2d
		lienzo= prueba1.getContext("2d");
		...
		prueba1.onmousedown=function() {pintar=true};
		//desactivamos pintar cuando baja el raton
		prueba1.onmouseup=function() {pintar=false}
		...
		prueba1.onmousemove=function(event) {
		...
		if ("ontouchstart" in document.documentElement) {
			   ...
			   prueba1.addEventListener("touchmove", function(event) {
	...
	function pinta(x,y,scrollsuperior) {
		...
		if (pintar) {
			...
			x=x-prueba1.offsetLeft;
			y=y-prueba1.offsetTop+scrollsuperior;
			var diferenciax=prueba1.offsetWidth-anchoinicial
			var porcentajeposicionx=x*100/prueba1.offsetWidth;
			x=x-(diferenciax*porcentajeposicionx/100)
			var diferenciay=prueba1.offsetHeight-altoinicial
			var porcentajeposiciony=y*100/prueba1.offsetHeight;
