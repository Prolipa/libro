...
var resp_cor_len3_act9_p13 = {
    0 : ["instrumento con el que antiguamente se hacían hilos.","img/i2_p13_act9.png"],
    1 : ["objeto puntiagudo que sirve para afinar y envolver los hilos.","img/i1_p13_act9.png"]
}
var cajas = document.getElementsByClassName('cajas');
var loadImg = ['img/i1_p13_act9.png','img/i2_p13_act9.png'];
var loadTxt = ['Instrumento con el que antiguamente se hacían hilos.','Objeto puntiagudo que sirve para afinar y envolver los hilos.'];
...
function initElementos(classDiv,arrayElement){       //Funcion para iniciar elementos como imagenes o texto en divs de acuerdo a un array
    ...
    for (i = 0; i < cajas.length; i++) {
        if(classDiv == "imagen"){
            $('#'+cajas[i].id).html('<img class="center img-responsive"  src="'+ arrayElement[i] +'" alt="' + arrayElement[i] + '" width="227px" height="180px"  id="img'+ [i] +'">');
...
function contarRespCorrectaDnD(){
    ...
    for(var i=0; i<Object.values(resp_cor_len3_act9_p13).length; i++){
        for(j=0; j<Object.values(cont_resp_cor).length;j++){
            if(JSON.stringify(Object.values(resp_cor_len3_act9_p13)[i]) === JSON.stringify(Object.values(cont_resp_cor)[j])){
                contador_nota = contador_nota+1;
            }
        }
    }
    calcularNotaGeneral(
        Object.values(resp_cor_len3_act9_p13).length,
        contador_nota,
